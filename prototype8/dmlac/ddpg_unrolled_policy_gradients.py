...
class ddpg_unrolled_policy_gradients2:
    def __init__(self, state_shape=[None, 100], action_shape=[None, 1], output_bound_low=[-1.], output_bound_high=[1.], learning_rate=.99, tau=.001):
        ...
        self.critic_solver = tf.train.AdamOptimizer(1e-3).minimize(self.closs, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'critic_source'))

        #Actor loss and optimizer
        self.predicted_reward = self.rmodel.build(self.states, self.action_out)#(?, 1)
        self.predicted_next_state = self.smodel.build(self.states, self.action_out)
        self.predicted_q = self.critic_src.build(self.predicted_next_state, self.actor_src.build(self.predicted_next_state))#(?, 1)
        self.policy = self.predicted_reward + self.learning_rate * self.predicted_q
        self.aloss = -tf.reduce_mean(tf.reduce_sum(self.policy, axis=-1))
        self.actor_solver = tf.train.AdamOptimizer(1e-4).minimize(self.aloss, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'actor_source'))

        '''
        #Parameter assertions
        params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'actor_source') +\
                 tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'critic_source') +\
                 tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'actor_target') +\
                 tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'critic_target')
...
def main():
    ddpg = ddpg_unrolled_policy_gradients2()
