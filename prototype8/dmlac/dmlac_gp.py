...
from gp import multivariate_gaussian_process
...
class dmlac_gp:
    def __init__(self, state_shape=[None, 2], action_shape=[None, 1], output_bound_low=[-1.],
                 output_bound_high=[1.], learning_rate=.9, tau=.01, forward_steps=1, trace_decay=.9, hist_len=15):
        ...
        self.smodel = multivariate_gaussian_process([None, self.state_shape[-1]+self.action_shape[-1]], self.state_shape)
        self.rmodel = multivariate_gaussian_process([None, self.state_shape[-1]+self.action_shape[-1]], [None, 1])
        ...
        #Tmp variables
        self.chol_in = self.squared_exponential_kernel(self.state_action, self.state_action) + tf.diag(5e-5 * tf.ones_like(tf.reduce_sum(self.state_action, axis=-1)))
        self.chol_out = tf.cholesky(self.chol_in)

        #Policy gradient
        self.mu = self.actor_src.build(self.states)
        self.reward_predict_mu = self.rmodel.build(self.state_action, tf.concat([self.states, self.mu], axis=-1), tf.expand_dims(self.rewards, axis=-1))
        self.state_predict_mu = self.smodel.build(self.state_action, tf.concat([self.states, self.mu], axis=-1), self.next_states)
        ...
        self.reward_predict = self.rmodel.build(self.state_action, tf.concat([self.states, self.actions], axis=-1), tf.expand_dims(self.rewards, axis=-1))

        #State model squared loss
        self.state_predict = self.smodel.build(self.state_action, tf.concat([self.states, self.actions], axis=-1), self.next_states)
    ...
    def update_hist(self, states, actions, rewards, next_states):
        self.hist_state_action += list(np.concatenate([states, actions], axis=-1))
        self.hist_reward += list(rewards)
        self.hist_next_state += list(next_states)

        self.hist_state_action = self.hist_state_action[-self.hist_len:]
        self.hist_reward = self.hist_reward[-self.hist_len:]
        self.hist_next_state = self.hist_next_state[-self.hist_len:]
    ...
    def train(self, sess, states, actions, rewards, next_states, dones, *_):
        ...
        self.update_hist(states, actions, rewards, next_states)


        chol_in = sess.run(self.chol_in, feed_dict={self.state_action:np.stack(self.hist_state_action)})
        import pickle
        pickle.dump( chol_in, open( "save4.p", "wb" ) )
        sess.run(self.chol_out , feed_dict={self.state_action:np.stack(self.hist_state_action)})


        #Train policy function
        sess.run(self.actor_opt, feed_dict={self.state_action:np.stack(self.hist_state_action),
