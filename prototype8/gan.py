...
class CGAN():
    def __init__(self, input_shape=[None, 4], action_size=4, latent_size=4, gen_input_shape=[None, 4], continuous_action=False):
        ...
        if self.continuous_action == False:
            ...
            self.prior = tf.concat([self.states, self.actions], axis=1)
        ...
        # Build the GAN
        self.G_sample = self.generator(self.Z, self.prior)
        self.D_real = self.discriminator(self.X, self.prior)
        self.D_fake = self.discriminator(self.G_sample, self.prior)
    ...
    def init_second_stream(self):
        ...
        if self.continuous_action == False:
            ...
            self.prior2 = tf.concat([self.states2, self.actions_onehot2], axis=1)
        else:
            self.actions2 = tf.placeholder(shape=[None, self.action_size], dtype=tf.float32)
            self.prior2 = tf.concat([self.states2, self.actions2], axis=1)

        self.Z2 = tf.placeholder(shape=[None, self.latent_size], dtype=tf.float32)
        self.G_sample2 = self.generator(self.Z2, self.prior2)
