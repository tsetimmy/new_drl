...
def main():
    ...
    assert args.model in ['mlp', 'gan', 'gated', 'dmlac_mlp', 'dmlac_gan', 'dmlac_gated', 'ddpg_unrolled_pg_mlp', 'dmlac_gp', 'dmlac_truth', 'mpc']
...
def init_model(input_shape, action_size, latent_size, learning_rate, action_bound_low, action_bound_high, tau, model):
    if model == 'gan':
        ...
        from dmlac.model_predictive_control import model_predictive_control 
        jointddpg = model_predictive_control(input_shape=input_shape, action_shape=[None, action_size], action_bounds=list(action_bound_low)+list(action_bound_high), learning_rate=learning_rate)
    ...
    return jointddpg, None, None, None, None
