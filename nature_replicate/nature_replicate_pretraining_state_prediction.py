...
class network():
    def __init__(self, input_shape, a_size, scope=None):
        with tf.variable_scope(scope):
            ...
            #---#
            self.S = slim.fully_connected(self.fc1, 84 * 84 * a_size, activation_fn=None)
...
def main():
    ...
    parser.add_argument("--pretraining-data-samples", type=int, default=0)
    parser.add_argument("--pretraining-batch-size", type=int, default=0)
    ...
    mode = 'generate'
    if args.pretraining_epochs > 0 and 'generate' in mode:
        dynamics_data = generate_dynamics(environment, args.pretraining_data_samples, args.state_len_max, args.action_size)
        if mode == 'generate_and_save':
            with open('dynamics_data' + str(args.pretraining_data_samples) + '.p', 'wb') as handle:
                pickle.dump(dynamics_data, handle, protocol=pickle.HIGHEST_PROTOCOL)
    elif mode == 'load':
        with open('dynamics_data' + str(args.pretraining_data_samples) + '.p', 'rb') as handle:
    ...
    with tf.Session() as sess:
        ...
        sess.run(update_ops)
        losses = []
        for d in range(args.pretraining_epochs):
            dd_batch = np.array(dynamics_data.sample(args.pretraining_batch_size))
            print len(dd_batch)
        ...
        for j in range(args.epochs):
            ...
            while done == False:
                ...
                if steps > args.replay_start_size:
                    ...
                    dynamics_pred = sess.run(qnet.S_reshape, feed_dict={qnet.image_in:states})
                    assert len(dynamics_pred) == len(actions)
                    assert len(dynamics_pred) == len(states1)
                    for i in range(len(dynamics_pred)):
                        dynamics_pred[i, :, :, actions[i]] = (states1[i, :, :, -1].astype(np.float32) - states1[i, :, :, -2].astype(np.float32)) / 255.

                    _, dynamics_l = sess.run([qnet.update_model2, qnet.loss2], feed_dict={qnet.image_in:states, qnet.targetS:dynamics_pred})
                    total_losses_dynamics += dynamics_l

                    #Training step: q-values
                ...
                if steps % args.eval_freq == 0:
                  print "evaluating..."
                  eval_score = evaluate(sess, args.environment, qnet, args.action_size, args.state_len_max)
                  print "eval_score:", eval_score

                if done == True:
                    print "j:", j, "total rewards", total_rewards, "total losses", total_losses, "total dynamics losses", total_losses_dynamics
                    break
    env.close()
